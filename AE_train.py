...
def AE_model_train(hyp, traindataset):
    ...
    # 保存模型
    torch.save(model.state_dict(), os.path.join('./models', '{}_{}_AE.pth'.format(best_epoch, best_loss)))
    model_path = os.path.join('./models', '{}_{}_AE.pth'.format(best_epoch, best_loss))

    f = open("./resultLog.txt", "a")
    print('AutoEncoder_model_path:', model_path, file=f)
    f.close()
    ...
    ax.set_ylabel('Loss')
    loss_path = os.path.join('./result/{}_{}_loss_AE.png'.format(best_epoch, best_loss))
    plt.savefig(loss_path)

    f = open("./resultLog.txt", "a")
    print('AutoEncoder_loss_png:', loss_path, file=f)
    f.close()
